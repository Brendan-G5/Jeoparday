...
function DataText({ data, changeSelector }) {
  ...
  function checkCurrentStreak (dateList) {
    ...
    if (dateList[0] === current) {
      ...
      for (let i = 1; i<dateList.length; i++) {
        if (dateList[i] === moment(current, "DD-MM-YYYY").subtract(1, "days").format("DD-MM-YYYY")) {
          current = moment(current, "DD-MM-YYYY").subtract(1, "days").format("DD-MM-YYYY")
          currentStreak++
        } else {
          return;
        }
  ...
  return (
    <div className="data-text">
      ...
      <div className="text-best">Longest Streak: <span className = "import-num">{bestStreak}</span></div>
